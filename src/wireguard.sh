...
log "Install wireguard on ${distro} ${version}"
...
if [ -z ${WIREGUARD_PORT+x} ]; then
    ...
    log "WIREGUARD_PORT is unset. Using ${WIREGUARD_PORT}"
fi

if [ -z ${WG0_IP+x} ]; then
    WG0_IP=10.10.10.1/24
    log "WG0_IP is unset. Using ${WG0_IP}"
fi
...
log "Wireguard setting file generation"
...
log "Systemd integration"

systemctl enable wg-quick@wg0
systemctl start wg-quick@wg0

log "Networking and Firewall Configuration"

sed -i 's/.*net.ipv4.ip_forward=.*/net.ipv4.ip_forward=1/' /etc/sysctl.conf
sysctl -p /etc/sysctl.conf

ufw allow $WIREGUARD_PORT/udp || log "Failed to allow port ${WIREGUARD_PORT} using ${RED}UFW${NC}" warn
